  ...
  return (
    <div className="cr2-war-banner" data-paused={Boolean(activeBlackSwan)}>
      <div className="cr2-war-banner__head">
        <strong>⚔ 경제 전쟁 · {activeEconomicWar.name}</strong>
        <span>남은 기간: {activeEconomicWar.floorsLeft === 999 ? '최종전' : `${activeEconomicWar.floorsLeft}층`}</span>
      </div>
      ...
      {activeBlackSwan && <p className="cr2-war-banner__pause">블랙스완으로 경제전쟁 효과가 일시 정지 중입니다.</p>}
    </div>
  )
}
